...
def parse_search(parsed_dict):
    for key, value in parsed_dict['photos'].items():
        if(type(value) is list) :
            for item in value:
                secret = item['secret']
                if secret not in photo_secrets and len(photos) < 24:
                    photo_secrets.append(secret)
...
def main():
    try:
        ...
        for shop in coffeeshops :
            shop = dict(shop)
            search_photos_coffee(shop['shop_latitude'], shop['shop_longitude'])

        locations = db.execute('SELECT * FROM Scenic').fetchall()
        for loc in locations:
            loc = dict(loc)
            search_photos_scenic(loc['scenic_latitude'], loc['scenic_longitude'])
